...
@router.post("/criar_empresa", response_model=Empresa, status_code=201)
def create_empresa(payload: EmpresaCreate):
    return service.create_empresa(payload)

@router.get("/listar_empresas", response_model=list[Empresa])
def list_empresas():
    return service.list_empresas()

@router.get("/empresa_{id}", response_model=Empresa)
